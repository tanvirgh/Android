...
public class RoundKnobButton extends RelativeLayout implements
	...
	interface RoundKnobButtonListener {
		...
		// public void onScroll(int percentage);
	}
	...
	public RoundKnobButton(Context context, int back, int rotoron,
		...
		// lp_ivKnob.addRule(RelativeLayout.ALIGN_PARENT_TOP);
		// lp_ivKnob.addRule(RelativeLayout.CENTER_VERTICAL);
		// lp_ivKnob.addRule(RelativeLayout.CENTER_HORIZONTAL);
	...
	public void setRotorPosAngle(float deg) {
		...
		if (deg >= 210 || deg <= 150) {
			...
			matrix.postRotate(deg, m_nWidth / 2, m_nHeight / 2);// getWidth()/2,
																// getHeight()/2);
